...
def test_cache_fields_reference_the_canonical_definitions_not_a_local_copy():
    ...
    surfaces = {
        "observation_schema": _schema("observability", "observation_schema.json")["properties"],
        "cost_user_usage_item_schema": _schema("costs", "cost_user_usage_item_schema.json")[
            "properties"
        ],
    }
...
def test_every_ingest_depth_level_carries_the_cache_fields():
    ...
    """
    definitions = _schema("observability", "observation_ingest_schema.json")["definitions"]
...
def test_unreported_usage_fields_is_closed_unique_runtime_metadata():
    """The status list is a vocabulary, not a free-text telemetry channel."""
    definition = _schema("common_types_schema.json")["definitions"][
        "UnreportedUsageFields"
    ]
...
def test_ingest_rejects_unknown_or_duplicate_unreported_usage_fields(
    unreported_fields,
):
    validator = SchemaValidator()
    ...
    errors = validator.validate_json(
        {
            "id": "obs_bad_usage_status",
            "type": "generation",
            "name": "provider.call",
            "unreported_usage_fields": unreported_fields,
        },
        "observation_ingest_schema",
    )

    assert errors != [], "usage-status typos and duplicates must not validate"
...
def test_input_tokens_states_the_disjointness_convention_it_now_assumes():
    ...
    for args in (
        ...
        assert "before this contract lands" in description, (
...
def test_ingest_contract_marks_backend_rollout_as_not_yet_asserted():
    ingest = _schema("observability", "observation_ingest_schema.json")

    assert ingest["x-asserted-against-backend"] is False
    assert "TraigentBackend#2511" in ingest["$comment"]


def test_harness_disjointness_instruction_does_not_hide_unreported_cache_reads():
    harness = _schema("harness_session_record.json")
    description = harness["properties"]["model_usage"]["items"]["properties"][
        "input_tokens"
    ]["description"]

    assert "MUST NOT exclude cache reads unless" in description


def test_breaking_allowlist_is_scoped_to_exact_cached_token_properties():
    allowlist = json.loads(
        (REPO_ROOT / "scripts" / "breaking_schema_allowlist.json").read_text(
            encoding="utf-8"
        )
    )["entries"]
    entries = [entry for entry in allowlist if entry.get("pr") == "#379"]

    fields = (*USAGE_STATUS_FIELDS, "unreported_usage_fields")
    expected = {
        (
            "costs/cost_user_usage_item_schema.json",
            f"#/properties/{field}",
    ...
    for depth in range(6):
        segments = ["properties", "children", "items"] * depth
        for field in fields:
            expected.add(
                (
                    "observability/observation_ingest_schema.json",
                    "#/" + "/".join([*segments, "properties", field]),
                )
            )

    actual = {(entry["file"], entry.get("pointer_prefix")) for entry in entries}
    assert actual == expected
    ingest_reasons = [
        entry["reason"]
        for entry in entries
        if entry["file"] == "observability/observation_ingest_schema.json"
    ]
    assert all("response consumer" not in reason for reason in ingest_reasons)
    assert all("backend ObservationPayload" in reason for reason in ingest_reasons)
