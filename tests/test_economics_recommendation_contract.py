...
def _response(**extra) -> dict:
    event = {
        ...
        "payback": {
            ...
            "explanation": [
                {
                    "component": "avoided_bad_promotion_payback",
                    "usd_value": 100.0,
                    "number_value": 1.2,
                    "unit": "count",
                    "emphasis": "lower_bound",
                },
            ],
        },
...
def test_route_binds_the_recommendation_request_schema() -> None:
    validator = _v()
    assert (
        validator._endpoint_schemas.get(f"POST:{ENDPOINT}") == REQUEST
...
def test_route_remains_contract_first_at_operation_level() -> None:
    ...
    """
    assert (
        SchemaValidator(contract="backend")._endpoint_schemas.get(f"POST:{ENDPOINT}")
        == REQUEST
    )
...
def test_endpoint_documents_success_auth_validation_and_unavailable() -> None:
    ...
    assert responses_200_ref(op).endswith(
        "economics_recommendation_response_schema.json"
    )
...
def test_every_error_status_binds_the_shared_error_envelope() -> None:
    ...
    responses = _load("economics_endpoints.json")["paths"][ENDPOINT]["post"][
        "responses"
    ]
...
def test_request_requires_stable_contract_and_version_identifiers() -> None:
    assert _rejected(
        _request(contract="economics_telemetry"), REQUEST
...
def test_request_rejects_unknown_and_tenant_asserting_fields() -> None:
    ...
    assert _rejected(
        _request(tenant_id="t-other"), REQUEST
    ), "a client-asserted tenant field must not be representable"
    assert _rejected(
        _request(project_id="p-other"), REQUEST
...
def _transmit(field: str, value, *reports, allowlist=None, container="bands") -> dict:
    ...
    body["sharing_policy"]["allowlist"] = (
        list(allowlist) if allowlist is not None else [field]
    )
    return body
...
def test_inferred_value_must_account_for_its_evidence() -> None:
    ...
    assert _rejected(
        body, REQUEST
    ), "an inferred value with no evidence status is a guess"
...
def test_inferred_evidence_may_be_withheld_but_not_leaked() -> None:
    ...
    body["characterization"]["field_reports"][0][
        "evidence_pointer"
...
def test_a_transmitted_value_must_be_on_the_sharing_allowlist() -> None:
    ...
    assert _rejected(
        off_allowlist, REQUEST
    ), "a value off the sharing allowlist must not egress"
...
def test_the_allowlist_binding_covers_overrides_not_only_bands() -> None:
    off = _transmit(
        ...
        _report(
            ...
            confidence=0.8,
            evidence_status="provided",
            evidence_pointer="traces_14d_count",
        ),
        allowlist=[],
        container="overrides",
    )
    assert _rejected(
        off, REQUEST
    ), "a transmitted override off the allowlist must be rejected"
    on = _transmit(
        "observed_daily_volume",
        80,
        _report(
        ...
        allowlist=["observed_daily_volume"],
        container="overrides",
    )
...
def test_every_vocabulary_field_is_allowlist_gated_not_just_the_sampled_ones() -> None:
    ...
    for field in sorted(all_fields):
        ...
        report = _report(
            field, provenance="asked", confidence=1.0, sharing_outcome="shared"
        )
        off = _transmit(field, value, report, allowlist=[], container=container)
        assert _rejected(
            off, REQUEST
        ), f"{field}: transmitted off-allowlist was accepted"
        on = _transmit(field, value, report, allowlist=[field], container=container)
        assert _ok(
            on, REQUEST
        ), f"{field}: honest allowlisted transmission was rejected"
...
def test_allowlist_is_bounded_and_drawn_from_the_vocabulary() -> None:
    ...
    assert _rejected(
        body, REQUEST
    ), "the allowlist is drawn from the closed field vocabulary"
    body = _request()
    body["sharing_policy"]["allowlist"] = [
        "value_channel",
        "value_channel",
...
def test_the_request_carries_no_presentation_string_at_all() -> None:
    ...
    assert _rejected(
        _request(agent_display_name="your QA agent"), REQUEST
...
def _free_text_paths(schema: dict) -> set[str]:
    ...
    def deref(
        node: dict, doc: dict, seen: frozenset
    ...
    def walk(node: dict, doc: dict, path: str, seen: frozenset) -> None:
        ...
        if admits_string(node, doc, seen):
            ...
            return
        for sub in (
            s for key in ("allOf", "anyOf", "oneOf") for s in node.get(key, [])
        ):
...
def test_the_redaction_obligation_names_every_content_capable_string() -> None:
    ...
    for path in EXPECTED_CLIENT_AUTHORED_STRING_PATHS:
        ...
        assert any(
            f"`{path}`" in o for o in redacting
        ), f"{path} is not covered by any obligation that requires redaction"


def test_the_content_capable_string_surface_is_exactly_the_documented_inventory() -> (
    None
):
    ...
    assert (
        _free_text_paths(_load(f"{REQUEST}.json"))
        == EXPECTED_CLIENT_AUTHORED_STRING_PATHS
    )
...
def test_the_pointer_narrowing_bounds_the_channel_and_does_not_seal_it() -> None:
    ...
    for still_valid in (
        ...
        assert _ok(
            _pointer_request(still_valid), REQUEST
    ...
    assert (
        "does NOT make PII or values unrepresentable" in obligations
...
def test_response_requires_the_whole_recommendation_record() -> None:
    for field in (
        ...
        assert _rejected(
            body, RESPONSE
        ), f"{field}: an incomplete recommendation is not valid"
...
def test_assumptions_are_labelled_starting_not_validated() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "the starting-assumption label cannot be set false"


def test_no_why_component_can_claim_an_assumption_was_validated() -> None:
    components = set(
        _load(f"{RESPONSE}.json")["definitions"]["WhyComponent"]["properties"][
            "component"
        ]["enum"]
    )
    assert "assumptions_are_starting_not_validated" in components
    assert not any(
        "validated" in c and c != "assumptions_are_starting_not_validated"
        for c in components
    ), "no render token may claim assumptions are validated"
    body = _response()
    body["why"][0] = {"component": "assumptions_validated"}
    assert _rejected(
        body, RESPONSE
    ), "a 'validated' render token must be unrepresentable"
...
def test_budget_floor_and_cap_are_labelled_starting_assumptions_not_funding() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "the floor/cap starting-assumption label cannot be set false"
...
def test_payback_is_only_claimed_on_a_positive_lower_bound() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "a positive lower bound must state its payback_days"
    ...
    assert _ok(
        body, RESPONSE
    ), "a non-positive lower bound is a valid, honest recommendation"
    body["payback"]["payback_days"] = 4.2
    assert _rejected(
        body, RESPONSE
    ), "payback must not be claimed on a non-positive lower bound"
...
def test_payback_daily_value_is_signed() -> None:
    ...
    assert _ok(
        body, RESPONSE
    ), "the conservative daily value may legitimately be negative"
...
def test_payback_explanation_is_structured_not_free_form() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "the payback explanation is structured tokens, not prose"
    body = _response()
...
def test_spend_zero_case_is_always_present_and_available() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "the spend-$0 case is required on every recommendation"
    body = _response()
...
def test_why_components_are_a_closed_vocabulary_referencing_user_fields() -> None:
    ...
    body = _response()
    body["why"][0] = {
        "component": "daily_volume_drives_scale",
        "references_field": "daily_volume_band",
    }
    assert _ok(body, RESPONSE)
    body["why"][0]["references_field"] = "internal_revenue_forecast"
    assert _rejected(
        body, RESPONSE
    ), "references_field is drawn from the closed field allowlist"
...
def test_stop_rule_and_required_receipt_are_closed() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "the required receipt kind reuses the closed receipt enum"
...
def test_response_money_is_finite_nonnegative_and_bounded() -> None:
    ...
    assert _rejected(
        body, RESPONSE
    ), "a negative recommended budget is not representable"
    body = _response()
        ...
        if isinstance(node, dict):
            ...
            if follow_refs and key is not None and key not in seen_refs:
                ...
                try:
                    ...
                    KeyError,
                    FileNotFoundError,
                ):  # pragma: no cover - fails loudly below
                    raise AssertionError(
                        f"unresolvable $ref while scanning: {ref}"
                    ) from None
...
def test_no_credit_incentive_or_pricing_field_is_declared_anywhere() -> None:
    ...
    for schema_name in (REQUEST, RESPONSE):
        ...
        assert (
            not offenders
        ), f"{schema_name}: forbidden WI-D/pricing field(s): {offenders}"
...
def test_request_reuses_vocabulary_and_telemetry_by_ref_not_restatement() -> None:
    ...
    assert (
        "economics_run_event_schema.json#/definitions/CharacterizationTelemetry" in blob
    )
    # the band enum members are NOT restated in the request file itself
    assert (
        "save_expert_time" not in blob
    ), "value-channel enum members must not be restated"
...
def test_response_reuses_common_and_vocabulary_by_ref() -> None:
    ...
    assert (
        "economics_characterization_vocabulary_schema.json#/definitions/ValueChannel"
        in blob
    )
    # archetype enum members are not restated here
    assert (
        "solo_coding_builder" not in blob
    ), "archetype enum members must not be restated"
...
def test_the_all_withheld_pair_leads_with_spend_zero_and_no_payback() -> None:
    ...
    assert (
        "payback_days" not in resp["payback"]
    ), "no payback on a non-positive lower bound"
...
def test_an_undetermined_recommendation_cannot_invent_a_value_channel() -> None:
    ...
    ), "the honest all-withheld answer must be representable"
    ...
    assert _rejected(
        determined_without_channel, RESPONSE
...
def test_the_all_withheld_vector_pair_does_not_reconstruct_withheld_values() -> None:
    ...
    assert not request["characterization"].get(
        "bands"
    ), "an all-withheld request shares no band"
    assert response["archetype"] == "undetermined"
    assert (
        "dominant_value_channel" not in response
    ), "a withheld channel was reconstructed"
...
def test_the_value_interval_must_be_denominated_in_dollars() -> None:
    ...
    for wrong_unit in ("tokens", "seconds", "count"):
        ...
        assert _rejected(
            body, RESPONSE
        ), f"a {wrong_unit} interval was accepted as a USD basis"
    inverted = _response()
    inverted["budget"]["basis"]["value_lower_bound"].update(
        {"lower": 900.0, "estimate": 100.0}
    )
...
def test_unenforceable_invariants_are_declared_as_backend_obligations() -> None:
    ...
    for marker in (
        "TENANT/PROJECT FROM CONTEXT",
        "SHARING POLICY IS CLIENT-ENFORCED",
        "NO PRESENTATION CHANNEL",
        "EVIDENCE POINTERS ARE OPAQUE, NOT SEALED",
        "CLIENT-AUTHORED STRINGS ARE USER CONTENT",
    ...
    for marker in (
        "BUDGET ORDERING",
        "LOWER-BOUND LEADS",
        "NO PRICING/CREDIT/WALLET INPUT",
        "PAYBACK ONLY ON A POSITIVE LOWER BOUND",
        "ASSUMPTIONS ARE NOT VALIDATED",
        "VALUE INTERVAL ORDERING",
        "REQUEST ID IS AN ECHO, NOT A NEW ID",
